...
const KINDS: ReadonlyArray<readonly [string, IdKind<string>]> = Object.entries(ID_KINDS);
...
describe('ID_PREFIXES', () => {
  ...
  it('covers exactly the kinds in ID_KINDS', () => {
    ...
    for (const [name, kind] of KINDS) {
      expect(kind.prefix).toBe(ID_PREFIXES[name as keyof typeof ID_PREFIXES]);
    }
  });
});
...
describe('schemas', () => {
  it('compose into an object schema', () => {
    ...
    expect(SendSchema.safeParse({ projectId: senderAgentId, senderAgentId }).success).toBe(false);
  });
...
describe('branding (checked by tsc, not at runtime)', () => {
  it("keeps one kind of id out of another's slot", () => {
    ...
    expect(takesAgent('agt_018f6b1a-9c2e-7f3a-8b4d-5e6f70819a2b')).toBeTypeOf('string');
