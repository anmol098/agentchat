 ...
 * | 1    | A migration failed. The transaction rolled back. |
 * | 65   | The database is newer than this image (`EX_DATAERR`) |
 * | 69   | The database is unreachable, or the lock was held too long (`EX_UNAVAILABLE`) |
 * | 78   | The configuration or the image itself is wrong (`EX_CONFIG`) |
...
function exitCodeFor(error: unknown): number {
  ...
  if (error instanceof MigrationFailedError) {
    // A migration that could not even reach the database is an availability
    // problem the next attempt may well survive, and telling it apart from a
    // broken migration is the difference between retrying and rolling back.
    return isConnectionFailure(error.cause) ? EXIT_UNAVAILABLE : EXIT_FAILURE;
  }
  return EXIT_FAILURE;
}

/** Whether a driver error means "could not talk to Postgres". */
