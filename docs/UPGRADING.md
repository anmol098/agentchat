...
**Status:** Operator manual. Written for someone running their own instance, who
...
`postgres` service. If you run something else, the shapes still apply — the
substance is which image tag runs, and which schema it finds when it starts.

**The one thing to know before anything goes wrong:** rolling back to an older
release requires an extra variable, `AGENTCHAT_ALLOW_SCHEMA_AHEAD=true`. Without
it the rollback fails, every time, with exit 65, and it looks exactly like a
compatibility break. It is not. It is a missing flag. See [Rolling
back](#rolling-back).
...
One version number covers the whole project — server, CLI, client library and
protocol are released together as `vX.Y.Z`, so "which CLI works with which
server" has an answer. The promise is:

| | Guaranteed |
|---|---|
| **Upgrade one minor version** (`1.2` → `1.3`) | Yes. Migrations apply on the way up; no manual step. |
| **Upgrade across several minors** (`1.2` → `1.6`) | Yes. Migrations are cumulative and each is written against the one before it. |
| **Roll back one minor version** (`1.3` → `1.2`) | Yes, **with `AGENTCHAT_ALLOW_SCHEMA_AHEAD=true`**, and without restoring a backup. |
| **Roll back more than one minor** (`1.6` → `1.2`) | **No.** That is a restore from backup. |
| **Patch releases** (`1.3.0` → `1.3.1`) | Both directions, freely. |
...
| **Anything across a major version** (`1.x` → `2.0`) | Nothing above is promised. See below. |

Underneath it is a discipline on every schema change: add columns nullable or
with a default, never rename in place, and never drop a column until the release
after the one that stopped reading it. That is what makes the previous release
able to run against the newer schema at all.
...
- **The wire protocol.** A field may be removed or repurposed, and a frame type
  may change meaning. Within a major version only additions are permitted, and
  both sides ignore what they do not recognise; across one, that stops holding.
- **The minimum CLI version may jump.** Users on an older CLI are refused with a
  ...
  not after.
- **Environment variables may be renamed, removed, or given new meanings.** A
  major upgrade may require editing `.env` before the stack will start.
...
- **A separate migration step may be required**, rather than the schema moving
  as part of the ordinary start.
- **Long-running data backfills** may ship as their own command, called out in
  the release notes, so that a start does not sit on a table lock for minutes.
...
- Release notes carry a mandatory **Upgrade notes** section: which migrations are
...
1. **Read the release notes** for the target version, and for every version you
...
4. **Have a window.** Downtime is the container swap plus migration time, which
   is well under a minute for an ordinary release, but "well under a minute" is
   not "zero", and a long migration is announced in the release notes precisely
...
# b. What the migration actually did, in its own words.
sudo docker compose logs --since 10m migrate

# c. The application is serving and can reach its database — from another
#    machine, so this also proves DNS, the firewall and the certificate.
curl https://chat.your-company.example/healthz
# {"status":"ok","checks":{"database":"ok"}}

# d. The image running is the one you asked for.
sudo docker compose images server
```

Then do something real: send a message and receive it. A health check proves the
process is alive and the database answers a ping, which is not the same as the
product working.
...
> **`GET /version` does not answer in v0.1.0.** The endpoint is specified, and
> the handler is in the source, but nothing registers it on the running server —
> it returns `401`, and `agentchat version --server <url>` fails with
> `AUTH_REQUIRED`. Until that is wired up, `docker compose images server` is how
> you confirm which release is running. Nothing else depends on it.

A healthy migration log looks like this — four lines, and the counts are the
interesting part:

```json
{"msg":"migration run starting","bundledCount":4,"newestBundledTag":"0003_…"}
{"msg":"advisory lock acquired","lockWaitMs":1}
{"msg":"applying 1 migration(s)","pendingTags":["0003_…"]}
{"msg":"applied 1 migration(s) in 51 ms","appliedCount":1}
```

`"nothing to apply"` in place of the last two is also success: it means this
release shipped no new migration.
...
failure is terminal and legible instead of a crash loop.

### Rolling back more than one minor version

Do not. The one-version guarantee is what the expand/contract discipline buys;
two versions back, a column the older release needs may genuinely have been
dropped, and `AGENTCHAT_ALLOW_SCHEMA_AHEAD=true` would let it start and then
fail on real traffic — which is worse than not starting.

Going further back is [a restore from
backup](#restoring-from-a-backup), which puts the schema back too.

---

## The migration exit codes

The migration program follows `sysexits.h` so that a human or a deploy script
can tell "try again" from "this will never work". This matters more than it
sounds: **exit 65 will never succeed on a retry**, and it is the most likely
upgrade failure, because it is what a rollback looks like.
...
endless crash loop that can never succeed, burying the one message that explains
what to do underneath a wall of identical restarts. If you deploy this image
...
having exited 0. If you run the image with `MIGRATE_ON_BOOT=true` instead — the
image's own default — the same check runs inside the server container before it
serves, and the container exits 65.
...
migration job, so that one line covers whichever shape you end up running.

---

## Restoring from a backup

This is the real way back from a multi-version rollback, a bad migration, or a
...
It is also destructive: everything written since the dump is gone. Read the whole
...
is an ordinary upgrade: the migration job applies whatever is missing on the way
up, and you do not need the opt-in for that direction.

---

## PostgreSQL major-version upgrades

Out of scope for AgentChat, and deliberately so: the application is an ordinary
PostgreSQL client and has no opinion about the server's major version. But it is
your problem eventually, so here is where it sits.

The Compose file pins **`postgres:18`**, to a major version, on purpose. Postgres
...
intact, unreachable — with an error most people meet for the first time at that
moment. Do not change that pin casually, and do not let a managed provider
...
| **`pg_upgrade`** | A database large enough that a dump/restore window is unacceptable. `--link` makes it near-instant and leaves no way back but a backup. | [`pg_upgrade`](https://www.postgresql.org/docs/current/pgupgrade.html), run inside a container that has **both** major versions' binaries |
...
3. Stop `postgres`. **Rename** the volume rather than deleting it, so the old
   ...
   point the `postgres` service at a new volume name and change the image tag to
   the new major.
...
5. Bring the rest up, verify as after any upgrade, and only then remove the old
   volume — a week later, not the same afternoon.
...
dump first. AgentChat itself needs no configuration change for a new Postgres
major.

If you have [moved to a managed database](./self-hosting.md#running-postgres-somewhere-else),
this is your provider's procedure instead — but the same rule holds: pin the
major, and choose the moment yourself.
...
- **A CLI newer than the server** works. It prints a single warning line to
  stderr and carries on; any flag the older server does not know is ignored.
- **A CLI older than the server's stated floor** is refused, with an error
  naming the exact command to run. Each release's notes carry that floor, so
  when it moves, tell people before you upgrade rather than after.
...
message sent during the restart is delivered afterwards rather than lost. What a
user sees is a pause, not an error; what they must not see is a reason to
restart their agent by hand.
