...
- 2026-09-12 — v0.2.1 released, carrying the presence fix (T-069) the dogfood found: a live listener read as offline a minute after connecting. Branch protection is set (blocker 3). Two defects the pipeline exposed on its first release (T-068) and the rehearsal that could not run (T-063) are fixed; the second release was rehearsed from its branch before the tag existed. Of the twelve claims, the two-machine and two-runtime rows rest on the maintainer's report of 2026-09-11; the week-long record the requirements ask for is still not written down, which is why T-511 stays open.
- 2026-09-13 — v0.3.0 released: an installable Skill (`skills/agentchat/`, T-512), a fix for a message-content trust and CLI-provenance finding a security review raised on it (T-513), discoverability through skills.sh — a badge and `npx skills add anmol098/agentchat` (T-514) — and, on a real global install of the CLI, an automatic best-effort install of that Skill (T-515). Minor, not a patch: new backward-compatible capability, wire protocol and schema unchanged (`PROTOCOL_VERSION` stays 5, `MIN_CLIENT_VERSION` stays 0.1.0). The version bump's own dry-run rehearsal (T-516) caught a real defect before the tag existed: the postinstall script broke the server image's dependency-install stages, which build from copied manifest files alone, before `packages/cli/scripts/` is ever present. Fixed by guarding the postinstall command on the script file's existence, verified by reproducing that manifests-only tree by hand and by a full local `docker build`, then rehearsed clean a second time before merging.
