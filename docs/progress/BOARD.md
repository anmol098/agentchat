...
| [T-020](./tasks/T-020.md) | A rate-limit error code | In progress | subagent-T-020 | T-103 |
...
| [T-054](./tasks/T-054.md) | A socket reset during a token refresh kills the listener outright | Blocked | subagent-T-054 | — |
...
- **T-054** — A socket reset during a token refresh kills the listener outright: The fix is in packages/client/src/websocket/socket.ts, which T-054 does not declare: SocketStream must settle its refusal on 'error' when no 'close' can follow. No in_progress task owns that file; needs the path added.
