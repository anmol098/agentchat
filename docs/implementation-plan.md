...
**Target:** v0.1 MVP as defined in PRD section 47 / section 55
...
| D3 | **Ack is agent-scoped, not session-scoped.** A message is `pending` for an agent+project until *any* session of that agent acks it. Per-session delivery rows exist for diagnostics only. | Sessions are ephemeral (created per `listen`), so replay must key on the durable identity. See section 4.3. |
...
| D12 | **`.agentchat/config.json` is committed** to the repo. It holds only `projectId`/`projectSlug`. | Every clone resolves the same project (PRD section 14). |
...
| D17 | **List responses are enveloped** as `{ items: [...] }`, not bare arrays. ~~Project slugs use the same grammar as agent names, `^[a-z0-9][a-z0-9-]{0,31}$`.~~ **The slug clause is superseded by D18** and is struck rather than deleted, so that a reader who followed it can see what changed. The envelope clause stands. | A bare array cannot carry a pagination cursor, so adding one would be a major-version change under D-additive rules. Decided during T-201, while no client existed and the cost was one level of nesting. The slug clause was decided in the same breath, for the ergonomic reason that one grammar is one thing for a user to learn instead of two. That reason was sound and the decision was still wrong, because the two grammars answer to different authorities; see D18. |
| D18 | **The project-slug grammar is the database's, not the agent-name grammar.** `^[a-z0-9](?:[a-z0-9]\|-(?=[a-z0-9])){0,31}$` — runs of lowercase alphanumerics joined by *single* hyphens, 1–32 characters, no leading or trailing hyphen. Supersedes the slug clause of D17. | D17's grammar accepted `payments-` and `a--b`; `projects_slug_format` has always refused both, so those slugs passed every client-side check and failed as a constraint violation at storage — a 500 for a request the server could see was malformed, which T-107 hit and had to work around in its own service. Narrowed in T-025 to the grammar the check constraint already spelled, which is the authority: it is compiled into the database and no amount of client-side agreement talks it out of a rejection. Decoupled from the agent-name grammar rather than narrowed alongside it, because that pattern is stated verbatim in section 2 and transcribed verbatim into `agents_name_format`, so agent names had no such fault to fix. `PROTOCOL_VERSION` went to 3 with ten entries in the snapshot's breaking-change ledger. Same defect, same fix, as usernames in T-016. T-060 later narrowed the agent-name grammar to this same shape anyway, on an argument about speech rather than storage (D19), so the two sources are identical again — as a coincidence of conclusions, not a restored derivation. |
| D19 | **The agent-name grammar is the same shape as the other two handles.** `^[a-z0-9](?:[a-z0-9]\|-(?=[a-z0-9])){0,31}$` — runs of lowercase alphanumerics joined by *single* hyphens, 1–32 characters, no leading or trailing hyphen. Replaces the grammar section 2 stated verbatim, `^[a-z0-9][a-z0-9-]{0,31}$`. **The shape converges; the ceilings do not.** 32 for an agent name and a project slug, 39 for a username, because that one is GitHub's rule and not ours to choose. | A name has to survive a round trip through human speech. The workflow is not somebody typing an address: it is a user telling their harness *"check the implementation with alice's backend agent"*, and the harness querying the agent listing and resolving that phrase to `@alice/backend`. A name is spoken, matched, and passed along. `backend-` is none of those things — it cannot be said unambiguously, it reads as a typo, and it makes fuzzy resolution against a listing worse for nothing in return. Irregular shapes belong to the identifier instead: an agent already has `agt_<uuidv7>`, which is exact, opaque and never spoken, so anything that has to be precise already has somewhere to be precise. The name is the human half and should look human. Unlike D18 and T-016 this repairs nothing — `agents_name_format` transcribed `AGENT_NAME_PATTERN` character for character and still does, lookahead included, so no name has ever passed validation and died at storage. It is a product decision, taken in T-060 while it still costs a version bump and a paragraph rather than a rename path for real users. `PROTOCOL_VERSION` went to 4 with ten ledger entries; `agents_name_format` moved in the same release (`server/drizzle/0004_agent_name_single_hyphens.sql`) with no backfill, because none of the 502 agent names in the development database violated the narrower grammar. |
...
Deviations from PRD section 50: no `packages/daemon` (D1), no `packages/shared` (`protocol` is the shared package). `tests/` lives inside each package rather than at the root.
...
Matches PRD section 51 with the changes marked **(new)** or **(changed)**.
...
- `username` comes from GitHub login; `@alice/backend` = `users.username` + `/` + `agents.name`. Agent names: `^[a-z0-9](?:[a-z0-9]|-(?=[a-z0-9])){0,31}$` (D19).
...
- Every migration must follow the compatibility rules in section 12.3 so an operator can roll the server image back one version without touching the database.
- Authorization invariants (PRD section 31) enforced in the service layer:
...
**The three handle grammars are one shape and three rules** (D19, D18, T-016). They agree about shape — runs of lowercase alphanumerics joined by single hyphens, no leading or trailing hyphen — and disagree about length, and they are still three rules: each is pinned to its own check constraint, none derives from another, and each is free to move when its own authority moves. Verified in T-059 against a live database by attempting real inserts, not by reading regexes side by side — 390 candidates, exhaustive over `{a, 1, -}` to length five plus the length boundaries — and re-verified the same way in T-060 across the migration that narrowed the agent-name constraint:

| Handle | Protocol pattern | Database constraint | Ceiling |
|---|---|---|---|
| Agent name | `^[a-z0-9](?:[a-z0-9]\|-(?=[a-z0-9])){0,31}$` (D19) | `agents_name_format`, the same source string character for character — the lookahead included, since PostgreSQL's `~` uses advanced regular expressions | 32 on both sides, and the cap lives inside the regex |
...
**The shape is the same for all three, and that is now decided** (D19). `agent-` and `a--b` were valid agent names until T-060, while the same shapes were refused as project slugs and as usernames, so a user who typed all three handles met two different rules. That asymmetry was never a defect: agent names have never had the boundary-versus-storage fault slugs and usernames had, and there was nothing broken to repair. What settled it is what a name is *for*.

A name has to survive a round trip through human speech. The workflow is not somebody typing an address — it is a user telling their harness *"check the implementation with alice's backend agent"*, and the harness querying the agent listing and resolving that phrase to `@alice/backend`. A name is spoken, matched, and passed along, and `backend-` is none of those things: it cannot be said unambiguously, it reads as a typo, and it makes fuzzy resolution against a listing worse for nothing in return. Irregular shapes belong to the identifier instead. An agent already has `agt_<uuidv7>` — exact, opaque, never spoken — so everything that has to be precise already has somewhere to be precise. The name is the human half and should look human.

**The ceilings do not converge, and should not.** 32 for an agent name and a project slug; 39 for a username, because that number is GitHub's and the username is not ours to choose. Three rules that agree about shape is not the same thing as one rule, and nothing here derives from anything else: the agreement is a coincidence of conclusions, reached in three separate decisions on three separate days, and each pattern remains pinned to the check constraint that actually judges it.

The price was the third break of the wire contract, taken deliberately and while it is still cheap: `PROTOCOL_VERSION` 4, ten ledger entries, and a migration that moved `agents_name_format` with no backfill. Before a tag this costs a version bump and a paragraph; after one it would have cost a rename path for real users.
...
Auth
  ...
  GET  /me
  GET  /version                      → { version, protocolVersion, minClientVersion }   (unauthenticated, see section 12.4)
...
Projects
  ...
  GET  /invites/:code                → { project, invitedBy }        (preview before join, PRD section 27)
  POST /invites/:code/join
  DELETE /projects/:id/invites/:inviteId   → {}                      (revoke; any member, D11; idempotent)
  POST /projects/:id/leave
  GET  /projects/:id/agents          → [{ agent, owner, online, sessions: n }]   (discovery, PRD section 21)
...
`type` is a transport operation only (PRD section 34/section 35). There will never be a semantic frame type.
...
- Duplicates are possible (e.g. ack lost). Listener dedupes by `messageId` in-process; harness is told to treat `messageId` as idempotency key (PRD section 24).
...
Agent selection deliberately lives in *user* config, not the repo config: the repo is shared, the agent is personal (PRD section 15).
...
agentchat setup                               # wizard: login → create/join project → create agent → write .agentchat/config.json (PRD section 54)

agentchat project list
agentchat project create <name>
agentchat project invite [--expires 7d]       # prints code
agentchat project join <code>                 # preview + confirm (PRD section 27)
...
agentchat agents [--json]                     # discovery, grouped by owner, online/offline (PRD section 21)
...
### 6.3 `listen` behaviour (PRD section 38–section 40)
...
- Self-hosters point `serverUrl` at their own instance and register their own GitHub OAuth app; nothing in the protocol depends on GitHub. Their users reach it with `agentchat login --server <url>`, once — see section 6.1.
...
- CLI: `login/logout/whoami`, `project *`, `agent *`, `status`, context resolution (section 6.1), `--json`, error rendering, exit codes.
...
- WS endpoint, socket registry, delivery algorithm (section 4.3), heartbeat/stale marking.
...
- **Done when:** the PRD section 55 flow works on one machine with two users/agents/sessions.
...
The stdout/stderr contract test is non-negotiable: it is what makes harness integration reliable (PRD section 39).
...
|----------|--------|---------------|
| Hosting | Open-source, host is the user's choice. Reference deployment on a single EC2 VM via Docker Compose (Cloud Run rejected: WebSocket timeout cap and multi-instance fan-out). | D6, M5, section 12 |
| Upgrades for self-hosters | Versioned images, bundled forward-only migrations, N-1 compatible schema changes, protocol version negotiation. | section 12 |
| Who can invite | Owners and members. | D11, section 3 |
| Invite expiry | Not specified; plan default of 7 days, unlimited uses, revocable. | section 3 |
| Message size | 1 MiB, to be reduced when file sharing is designed. | D10, section 2 |
| Commit `.agentchat/config.json` | Yes. | D12 |
| Agent deletion | In scope, soft delete. | D13, section 2, section 3, section 6.2, M4 |
| Reading others' conversations | Follow plan: restricted to the caller's own agents. | D15 |
| Runtime detection | None. `listen --runtime` is required. | D14, section 6.3 |
...
- section 8/section 47/section 54: lowercase `agentchat` everywhere; `.agentchat/` not `.AgentChat/`.
- section 18: replace the placeholder with D2 (fan-out) and D3 (agent-scoped ack).
- section 41/section 42: mark daemon as v0.2; describe v0.1 as direct CLI↔server with a transport abstraction.
- section 51: add `message_inbox`, `project_invites`, `refresh_tokens`, `client_message_id`; mark `deliveries` as diagnostic.
- section 30: name GitHub device flow as the reference IdP while keeping the protocol IdP-agnostic.
- section 32: replace with section 3 of this document.
- section 38: add NDJSON status events and the reply hint line to the listener contract; `--runtime` is a required argument.
- section 47: add "delete agent" under Agents.
- section 23/section 24: state the 1 MiB content limit.
- section 26/section 27: any member may create invites.
...
- section 16/section 22: state that `send` without `--conversation` creates a new conversation, and `--reply-to` inherits the parent's conversation.
...
- On tag push, GitHub Actions:
  1. runs the full test matrix including the migration checks in section 12.6;
...
- Listeners tolerate the restart: sockets drop, the client reconnects with backoff, `hello` replays anything pending (section 4.4). Expected downtime is the container swap plus migration time, typically under 30 s.
