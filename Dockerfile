...
# than rebuilt per environment (Plan section 12.1), so everything an operator needs to
...
# this repository (Plan section 12.2).
#
...
# (Plan section 1). Running the image on the same major that developers and CI use
...
# cache). For a release artefact that section 12.1 promises is built once and promoted
...
#    (Plan section 12.2). The first migration is being generated by the schema task
...
# `MIGRATE_ON_BOOT` defaults to enabled, per Plan section 12.2 — the documented
...
# __drizzle_migrations, and exit non-zero on failure (Plan section 12.2). It needs no
