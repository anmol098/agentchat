...
function serializeFrontmatter(data) {
  const order = [
    'id', 'title', 'milestone', 'status', 'owner', 'branch', 'pr',
    'estimate', 'depends_on', 'paths', 'blocked_reason',
  ];
...
function needsQuotes(value) {
  return typeof value === 'string' && (value.includes(': ') || value.startsWith('[') || value.startsWith('#'));
}
...
function render(tasks) {
  ...
  lines.push('> Claiming, updating, and quality rules: [Subagent Protocol](../SUBAGENT-PROTOCOL.md).');
  lines.push('');
  lines.push('## Overall');
  lines.push('');
  lines.push('```text');
  lines.push(`[${'#'.repeat(filled)}${'.'.repeat(20 - filled)}] ${pct}%   ${done}/${total} tasks done`);
  lines.push('```');
  lines.push('');
  ...
  if (blocked.length) {
    lines.push('## Blocked');
    lines.push('');
    for (const t of blocked) lines.push(`- **${t.id}** — ${t.title}: ${t.blocked_reason ?? 'no reason recorded'}`);
    lines.push('');
  }
...
function check(tasks) {
  ...
  for (const t of tasks) {
    ...
    if (t.id && t.file !== `${t.id}.md`) errors.push(`${where}: filename must match id (${t.id}.md)`);
    ...
    if (!Array.isArray(t.paths) || t.paths.length === 0) errors.push(`${where}: must declare at least one owned path`);
    if (!Array.isArray(t.depends_on)) errors.push(`${where}: depends_on must be a list`);
    if (t.status === 'in_progress' && !t.owner) errors.push(`${where}: in_progress requires an owner`);
    if (t.status === 'blocked' && !t.blocked_reason) errors.push(`${where}: blocked requires blocked_reason`);
    if (!/^##\s+Acceptance criteria/m.test(t.body)) errors.push(`${where}: missing "## Acceptance criteria" section`);
...
const commands = {
  list(argv) {
    ...
    for (const t of out) {
      ...
      process.stdout.write(`${t.id}  ${t.milestone}  ${t.status.padEnd(11)}${owner.padEnd(16)} ${t.title}\n`);
    }
  },
  ...
  show(argv) {
    ...
    process.stdout.write(`status:    ${task.status}${task.blocked_reason ? ` (${task.blocked_reason})` : ''}\n`);
    process.stdout.write(`owner:     ${task.owner ?? '—'}\n`);
    process.stdout.write(`branch:    ${task.branch ?? '—'}\n`);
    process.stdout.write(`depends:   ${task.depends_on.join(', ') || '—'}${unmet.length ? `  (unmet: ${unmet.join(', ')})` : ''}\n`);
  ...
  claim(argv) {
    ...
    if (task.status !== 'todo') {
      fail(`${task.id} is ${task.status}${task.owner ? ` (owner: ${task.owner})` : ''}, not todo. Pick another task.`);
    }
    const unmet = unmetDeps(task, tasks);
    ...
    for (const other of tasks) {
      ...
      if (tasksCollide(task, other)) {
        fail(`${task.id} owns paths that ${other.id} (${other.status}, ${other.owner}) is already working on: ${collidingPaths(task, other).join(', ')}`);
      }
    }

    const slug = task.title.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '').split('-').slice(0, 5).join('-');
    ...
    process.stdout.write('  git push origin main   # this push is the lock; if it is rejected, you lost the race\n');
    process.stdout.write(`  git worktree add ../agentchat-${task.id} -b ${task.branch} origin/main\n`);
  },

  status(argv) {
    const id = argv[0] ?? fail('usage: status <ID> <status> [--pr N] [--reason "…"]');
    const next = argv[1] ?? fail(`usage: status <ID> <${STATUSES.join('|')}>`);
    if (!STATUSES.includes(next)) fail(`invalid status "${next}"; expected one of ${STATUSES.join(', ')}`);
    ...
    appendLog(task, `Status ${previous} -> ${next}${reason ? `: ${reason}` : ''}${pr ? ` (PR #${pr})` : ''}.`);
    saveTask(task);
    renderAndSave(loadTasks());
  ...
  plan() {
    ...
    if (active.length) {
      ...
      for (const t of active) process.stdout.write(`  ${t.id}  ${t.status.padEnd(11)} ${t.owner ?? '—'}  ${t.title}\n`);
      process.stdout.write('\n');
    }
    if (batch.length === 0) {
